...
@section('content')
  ...
  <div class="card">
    ...
    <div class="card-body">
      <form method="POST"
        action="{{ isset($employer) ? route('employes.update', [$employer, $employer->COD_AG]) : route('employes.store') }}"
        ...
        <!-- Section 1: Identité -->
        <h4
          style="font-size:13px;font-weight:800;color:var(--primary);margin-bottom:14px;text-transform:uppercase;letter-spacing:.8px;">
          ...
          <div class="form-group">
            ...
            <select name="Sit_Familiale" class="form-control form-select">
              ...
              @foreach(['Célibataire', 'Marié(e)', 'Divorcé(e)', 'Veuf/Veuve'] as $sit)
                <option value="{{ $sit }}" {{ old('Sit_Familiale', $employer->Sit_Familiale ?? '') == $sit ? 'selected' : '' }}>
                  {{ $sit }}</option>
              @endforeach
            </select>
          </div>
        ...
        <div class="form-grid">
          ...
          <div class="form-group">
            ...
            <div class="form-group" style="grid-column:1/-1;">
              <label class="form-label">Adresse (AR)</label>
              <input type="text" name="ADRESSE_AR" class="form-control"
                value="{{ old('ADRESSE_AR', $employer->ADRESSE_AR ?? '') }}" maxlength="255">
            </div>
        ...
        <div class="form-grid">
          ...
          <div class="form-group">
            ...
            <input type="text" name="RIB" class="form-control" value="{{ old('RIB', $employer->RIB ?? '') }}"
              maxlength="30">
          </div>
        </div>
